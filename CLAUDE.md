...
**Eccezioni legittime (non modificare):**
- `TRANSFER_KEYWORDS` in `src/adapters.py` — stringhe raw dei bonifici bancari, devono matchare la descrizione reale
- `pattern_file` in `config.yaml` — pattern per trovare i file XLS esportati dalla banca (nomi reali)
...
- **Non usare nomi personali** (`daniel`, `alessandra`) nel codice, config.yaml o DB — usare `persona1`/`persona2`. Eccezioni: `TRANSFER_KEYWORDS` in `src/adapters.py` e `pattern_file` in `config.yaml`
